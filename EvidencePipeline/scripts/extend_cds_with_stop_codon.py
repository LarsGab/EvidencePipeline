...
Strategy:
    ...
    - UTRs, introns, start/stop_codon, etc. are not kept.
...
def read_gff(gff_handle):
    """Parse GFF into gene, transcript, CDS and stop_codon structures."""
    ...
    return genes, transcripts, cds_by_tx, stops_by_tx, gene_children
...
def rebuild_gff(genes, transcripts, cds_by_tx, gene_children, out_handle):
    ...
    tx_bounds = {}  # tx_id -> (seqid, strand, min_start, max_end)
    for tx_id, cds_list in cds_by_tx.items():
        if not cds_list:
            continue
        ...
        tx_bounds[tx_id] = (seqid, strand, min(starts), max(ends))
    ...
    for gene_id, tx_ids in gene_children.items():
        for tx_id in tx_ids:
            if tx_id in tx_bounds:  # only keep transcripts w/ CDS
    ...
    # 2) compute gene bounds from their CDS-carrying transcripts
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "only gene/transcript/exon/CDS features. Missing gene/transcript "
    ...
    genes, transcripts, cds_by_tx, stops_by_tx, gene_children = read_gff(gff_handle)
    ...
    # rebuild GFF using only updated CDS
    rebuild_gff(genes, transcripts, cds_by_tx, gene_children, sys.stdout)
