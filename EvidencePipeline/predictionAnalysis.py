...
class PredictionAnalysis():
    ...
    def saveWithMissingSupport(self, outFile):
        out = open(outFile, "w")

        for transcript in self.transcripts.values():
            if transcript.missingSupport():
...
class Transcript():
    ...
    def missingSupport(self):
        if (self.start and not self.start.support) or \
           (self.stop and not self.stop.support) or \
           (len(self.introns) > 0 and not self.fullIntronSupport):
            return True
