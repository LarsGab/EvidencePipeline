...
import EvidencePipeline.evi_initializer as p_i
import EvidencePipeline.utility as util
import EvidencePipeline.prep_evidence as p_e
import EvidencePipeline.generate_hc_genes as hc
import EvidencePipeline.generate_hints as hints
...
def main():
    ...
    if cfg["mode"] in ['mixed', 'rnaseq', 'isoseq']:
        ...
        hc.from_transcript_to_genome(hc_single_gff, stringtie_gff3, transdecoder_fasta, 
                f"{cfg['output_path']}/training.gff", cfg['TRANSDECODER_UTIL'])
    else:
        logging.info("Using Miniprot training genes as training genes.")
        util.copy_file(f"{cfg['output_path']}/miniprot/miniprot_trainingGenes.gff", f"{cfg['output_path']}/training.gff")


    logging.info(f"------> Training genes file {cfg['output_path']}/training.gff was generated successfully!")
