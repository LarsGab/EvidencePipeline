# Parameters YAML Guide for EvidencePipeline
EvidencePipeline allows supplying input data and settings via a single YAML file.
You pass the YAML file to the pipeline using the wrapper:
```bash
./run_evidence_pipeline.py --params-yaml parameters.yaml --profile mycluster -c ~/nf_configs/mycluster.config
```
## Input Data
...
EvidencePipeline accepts local long and short read data, add the absolut paths to your files to :
...
| `scoring_matrix`           | `"${projectDir}/../conf/blosum62.csv"` | Amino acid substitution scoring matrix used by homology-based tools.                                         |
...
Tiberius can be run together with the evidence pipeline to produce a full genome annotation using ab initio and extrinsic evidence based predicitons. 
Make sure that you have GPU available and that the nextflow configuration is set up for GPU tasks (see below).
...
# Adapting EvidencePipeline to an HPC
